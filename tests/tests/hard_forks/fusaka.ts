 ...
 * - EIP-7825: Transaction Gas Limit Cap (16.78M gas per transaction)
...
// EIP-7825: Transaction gas limit cap (2^24 ≈ 16.78M gas)
...
describeDevNode('Fusaka Hard Fork - EIP-7825: Transaction Gas Limit Cap', (context) => {
  it('should accept transaction at gas limit cap (16.78M gas)', async function () {
  ...
  it('should reject transaction exceeding gas limit cap', async function () {
    try {
      const tx = await createTransaction(context, {
        ...ALITH_TRANSACTION_TEMPLATE,
        to: alith.public,
      ...
      expect(result.txResults[0]).to.satisfy((txResult: any) => {
        return txResult === null ||
          (txResult.receipt && Number(txResult.receipt.status) === 0) ||
          txResult.error !== undefined;
      });
    } catch (error: any) {
      // Expected to throw error for exceeding gas cap
...
describeDevNode('Fusaka Hard Fork - Integration Test: All EIPs', (context) => {
  it('should handle transaction with gas cap, MODEXP, and CLZ in single block', async function () {
    ...
    const gasCapTx = await createTransaction(context, {
      ...
      to: alith.public,
      value: '0x1',
      gas: Math.floor(TRANSACTION_GAS_CAP * 0.9),
      nonce: Number(nonce1),
    });
