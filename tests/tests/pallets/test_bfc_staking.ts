...
describeDevNode('pallet_bfc_staking - set controller', (context) => {
  ...
  it('should successfully replace controller account', async function () {
    ...
    const rawUnstakingNominations: any = await context.polkadotApi.query.bfcStaking.unstakingNominations(newAlith.address);
    const unstakingNominations = rawUnstakingNominations.unwrap().toJSON();
    expect(unstakingNominations.nominations.length).equal(1);
    let isUnstakingNominationFound = false;
    for (const nomination of unstakingNominations.nominations) {
      if (nomination.owner === charleth.address) {
        isUnstakingNominationFound = true;
        break;
      }
    }
    expect(isUnstakingNominationFound).equal(true);
