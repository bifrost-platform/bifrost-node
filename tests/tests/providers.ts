...
import {
  BlockHeaderOutput, JsonRpcResponse, Log, SubscriptionParams, Web3
} from 'web3';
...
export async function customWeb3Request(web3: Web3, method: string, params: any[]) {
  return await web3.requestManager.send({ method, params });
...
export interface Subscription<T> extends SubscriptionParams<T> {
  once: (type: "data" | "connected", handler: (data: T) => void) => Subscription<T>;
}

// Little helper to hack web3 that are not complete.
export function web3Subscribe(web3: Web3, type: "newBlockHeaders"): Subscription<BlockHeaderOutput>;
...
export const provideWeb3Api = async (port: number, protocol: "ws" | "http" = "http") => {
  const web3 =
    protocol == "ws"
      ? new Web3(new Web3.providers.WebsocketProvider(`ws://127.0.0.1:${port}`))
      : new Web3(new Web3.providers.HttpProvider(`http://127.0.0.1:${port}`));
