...
/// Opaque request identifier, generated by TrancheManager at request time (the
/// investor's Vault call routes through TrancheManager, which mints the ID and
/// emits DepositRequested/RedeemRequested — this holds on both Hub and Spoke
/// chains, so the ID exists before the request ever reaches the Hub) — this
/// pallet never generates its own.
