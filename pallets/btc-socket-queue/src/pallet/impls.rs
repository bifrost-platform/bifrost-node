	...
	H160: Into<T::AccountId>,
{
	fn verify_socket_message(msg: &UnboundedBytes) -> Result<(), DispatchError> {
		...
		let expected_token_idx0: &[u8] = if chain_id == 3068 {
			...
			// ]
			// NOTE: this is for stage(testbed) only
			// 0x00000003000000030000bfc0847018622152e78b1cd7a55d65024a1a079d90da
			&[
				0, 0, 0, 3, 0, 0, 0, 3, 0, 0, 0xbf, 0xc0, 0x84, 0x70, 0x18, 0x62, 0x21, 0x52, 0xe7,
				0x8b, 0x1c, 0xd7, 0xa5, 0x5d, 0x65, 0x02, 0x4a, 0x1a, 0x07, 0x9d, 0x90, 0xda,
			]
		} else {
			return Err(Error::<T>::InvalidSocketMessage.into());
		};
