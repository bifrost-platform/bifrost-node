	...
	H160: Into<T::AccountId>,
{
	fn verify_socket_message(msg: &UnboundedBytes) -> Result<(), DispatchError> {
		...
		let expected_token_idx0: &[u8] = if chain_id == 3068 {
			...
			// 0x00000003000000030000bfc03249548c713c9c75c003de2dc0e7bc8fa7b8eea5
			&[
				0, 0, 0, 3, 0, 0, 0, 3, 0, 0, 0xbf, 0xc0, 0x32, 0x49, 0x54, 0x8c, 0x71, 0x3c, 0x9c,
				0x75, 0xc0, 0x03, 0xde, 0x2d, 0xc0, 0xe7, 0xbc, 0x8f, 0xa7, 0xb8, 0xee, 0xa5,
			]
		} else {
			return Err(Error::<T>::InvalidSocketMessage.into());
		};
