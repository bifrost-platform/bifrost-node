...
pub mod v7 {
	...
	/// Migration V7: Clear OnFlightTransfers and PendingTransfers, reset all AssetCaps.on_flight_cap to zero.
	///
	...
	impl<T: Config> OnRuntimeUpgrade for V7<T> {
		fn on_runtime_upgrade() -> Weight {
			...
			if current == 7 && onchain == 6 {
				...
				log!(
					info,
					"cccp-relay-queue v7: cleared {} OnFlightTransfers, {} PendingTransfers, reset {} AssetCaps.on_flight_cap to zero ✅",
					on_flight_count,
					pending_count,
					asset_caps_count
				);
			} else {
				log!(warn, "Skipping cccp-relay-queue storage v7 💤");
			}
