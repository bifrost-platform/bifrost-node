...
pub mod v5 {
	...
	/// Migration V5: Clear OnFlightTransfers and PendingTransfers, reset all AssetCaps.on_flight_cap to zero.
	///
	...
	impl<T: Config> OnRuntimeUpgrade for V5<T> {
		fn on_runtime_upgrade() -> Weight {
			...
			if current == 5 && onchain == 4 {
				...
				log!(
					info,
					"cccp-relay-queue v5: cleared {} OnFlightTransfers, {} PendingTransfers, reset {} AssetCaps.on_flight_cap to zero ✅",
					on_flight_count,
					pending_count,
					asset_caps_count
				);
			} else {
				log!(warn, "Skipping cccp-relay-queue storage v5 💤");
			}
