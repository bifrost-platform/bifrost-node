...
pub mod v6 {
	...
	/// Migration V6: Clear OnFlightTransfers and PendingTransfers, reset all AssetCaps.on_flight_cap to zero.
	///
	...
	impl<T: Config> OnRuntimeUpgrade for V6<T> {
		fn on_runtime_upgrade() -> Weight {
			...
			if current == 6 && onchain == 5 {
				...
				log!(
					info,
					"cccp-relay-queue v6: cleared {} OnFlightTransfers, {} PendingTransfers, reset {} AssetCaps.on_flight_cap to zero ✅",
					on_flight_count,
					pending_count,
					asset_caps_count
				);
			} else {
				log!(warn, "Skipping cccp-relay-queue storage v6 💤");
			}
