...
interface BfcStaking {
    ...
    /// @dev Cancel the request to set the commission ratio of the given candidate
    /// Selector: c1f6309a
    function cancel_validator_commission_set() external;

    /// @dev Set the tier of the given candidate
    /// Selector: ed048bda
    /// @param more The additional amount self-bonded (set to 0 if additional bonding is not desired)
    /// @param tier The tier to be set (1: Basic, 2: Full)
    /// @param relayer The relayer account for cross-chain functionality (optional: only for Full tier)
    function set_validator_tier(
        uint256 more,
        uint256 tier,
        address relayer
    ) external;
    ...
    /// @dev Cancel the request to reset the paired controller account of the requested stash
    /// Selector: f0f529b3
    function cancel_controller_set() external;
